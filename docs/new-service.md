...
A full worked example deploying [Gitea](https://gitea.io/) with:

- Cloudflare Tunnel ingress at `gitea.example.com`
- Authentik ForwardAuth protection
- Longhorn PVC for repository data

**`k3s/manifests/gitea/namespace.yaml`**
...
**`k3s/manifests/gitea/deployment.yaml`**
...
**`k3s/manifests/gitea/service.yaml`**
...
**`k3s/manifests/gitea/ingress.yaml`**
...
spec:
  ...
  sourceRef:
    kind: GitRepository
    name: flux-system
  path: ./k3s/manifests/gitea
  targetNamespace: gitea
```

**`opentofu/cloudflare-tunnel.tf`** (edit the existing file):

```hcl
# 1. Add to the ingress list in cloudflare_zero_trust_tunnel_cloudflared_config.homelab
#    (before the catch-all http_status:404 entry):
{
  hostname = "gitea.${var.cloudflare_zone_name}"
  service  = "http://traefik.kube-system.svc.cluster.local:80"
},

# 2. Add a new DNS record resource:
...
```bash
# Commit everything — OpenTofu Apply runs automatically on push
git add k3s/manifests/gitea/ k3s/flux/apps/gitea.yaml k3s/flux/apps/kustomization.yaml opentofu/cloudflare-tunnel.tf
git commit -m "feat: add gitea service"
git push
```

Flux reconciles the Kubernetes manifests within 10 minutes (or immediately with `flux reconcile kustomization apps -n flux-system`). The OpenTofu Apply workflow applies the DNS record and tunnel routing automatically.

Finally, create the Proxy Provider + Application in Authentik as described in [Step 4](#step-4-add-authentik-protection).
